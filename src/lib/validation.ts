// Kept in sync with the Stellar `text` memo length limit, in case on-chain
// memo support is added later — see the note in api/walletApi.ts.
...
export function validateReference(reference: string): ValidationResult {
  ...
  if (trimmed.length > STELLAR_MEMO_MAX_LENGTH) {
    return { ok: false, error: `La referencia no puede tener más de ${STELLAR_MEMO_MAX_LENGTH} caracteres.` };
  }
  return { ok: true };
}
